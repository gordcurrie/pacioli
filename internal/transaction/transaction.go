...
type Store interface {
	...
	// ListDistinctAllSecurityIDsByUser returns the set of security IDs the user has any
	// transaction for, across all accounts. Used by PortfolioService to enumerate
	// held positions without scanning all securities.
	ListDistinctAllSecurityIDsByUser(ctx context.Context, userID int64) ([]int64, error)
