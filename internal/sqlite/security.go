...
func scanSecurity(s securityScanner) (*security.Security, error) {
	...
	if lastPriceStr.Valid && lastPriceStr.String != "" {
		...
		if err != nil {
			return nil, fmt.Errorf("security %d: parse last_price_cad %q: %w", sec.ID, lastPriceStr.String, err)
		}
		sec.LastPriceCAD = &p
	}
	if lastPriceDateStr.Valid && lastPriceDateStr.String != "" {
		...
		if err != nil {
			return nil, fmt.Errorf("security %d: parse last_price_date %q: %w", sec.ID, lastPriceDateStr.String, err)
		}
		sec.LastPriceDate = &t
	}
	return &sec, nil
